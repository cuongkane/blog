...
- **Slot**: A resource unit within a TaskManager that runs one parallel slice of the job. Each slot executes a complete operator chain (Source → Operator → Sink) and has its own RocksDB instance for state.
