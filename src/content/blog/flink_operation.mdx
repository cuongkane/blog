---
title: "Flink Operation Phase: Detailed Step-by-Step Explanation"
...
- **Slot**: A resource unit within a TaskManager that runs one parallel slice of the job. Each slot executes a complete operator chain (Source → Operator → Sink) and has its own RocksDB instance for state. The number of slots per TaskManager is configurable via `taskmanager.numberOfTaskSlots`.
...
- **emptyDir (Ephemeral Storage)**: Kubernetes-managed temporary storage attached to each TaskManager Pod. RocksDB writes SST files here during normal operation. This storage is deleted when the Pod terminates—S3 is the durable backup.

- **S3 (Checkpoint Storage)**: Durable object storage that holds checkpoint data for recovery. TaskManagers upload RocksDB SST files here, and JobManager writes the `_metadata` file to mark checkpoint completion. On failure recovery, state is downloaded from S3 to restore the job.

- **Kafka**: External message broker serving as data source and sink. KafkaSource consumes from input topics and tracks offsets in Flink state (not Kafka consumer groups). KafkaSink produces to output topics using transactions to achieve exactly-once delivery.
...
# Flink Operation Phase: Detailed Step-by-Step Explanation
...
The source deserializes raw Kafka bytes into Flink records (using the configured deserializer) and emits them downstream. Records flow through Flink's network stack via serialized buffers. For keyed streams, records are partitioned by key hash to ensure all records with the same key reach the same operator subtask.

Records are serialized into `NetworkBuffer` segments (32KB default), sent via Netty TCP connections between TaskManagers. Back-pressure propagates upstream if downstream operators are slow — the network buffers fill up, causing the source to slow down its poll rate.
